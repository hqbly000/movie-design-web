...
const c=await CDP.c(t.webSocketDebuggerUrl);await c.send('Page.enable');await c.send('Runtime.enable');await c.send('Network.enable')
const errs=[];c.ws.addEventListener('message',e=>{const m=JSON.parse(e.data);if(m.method==='Network.responseReceived'){const u=m.params.response.url;if(u.includes('/api/'))errs.push(m.params.response.status+' '+u.replace('http://127.0.0.1:5174',''))}})
await c.send('Page.navigate',{url:'http://127.0.0.1:5174/login'})
await new Promise(r=>setTimeout(r,1500))
await c.ev(`localStorage.clear(); sessionStorage.clear(); true`)
await c.send('Page.navigate',{url:'http://127.0.0.1:5174/login'})
await new Promise(r=>setTimeout(r,1500))
...
await c.ev(`location.href='/honors'`)
await new Promise(r=>setTimeout(r,4000))
const dump=await c.ev(`JSON.stringify({path:location.pathname,th:[...document.querySelectorAll('table thead th')].map(x=>x.innerText.trim()),rows:document.querySelectorAll('table tbody tr').length,hasTable:!!document.querySelector('table'),txt:document.body.innerText.slice(0,120)})`)
console.log('DUMP',dump)
console.log('API CALLS:',JSON.stringify(errs.slice(-12),null,1))
